# PowerShell script to recreate commits with the correct author details

Write-Host "Starting Git Repository Author Correction..." -ForegroundColor Cyan

# ----------------- SERVER REPOSITORY RE-CREATION -----------------
Write-Host "`nRe-initializing Server Repository..." -ForegroundColor Green
if (Test-Path "server/.git") {
...
# Configure correct Git credentials
git config user.name "raihanulR"
git config user.email "raihanul409@gmail.com"
...
# ----------------- CLIENT REPOSITORY RE-CREATION -----------------
Write-Host "`nRe-initializing Client Repository..." -ForegroundColor Green
cd ..
...
git add .
git commit -m "chore: track assets, layouts, hooks, and remaining project config files"

Write-Host "`nAll Git repositories re-initialized and committed under raihanul409@gmail.com!" -ForegroundColor Cyan
